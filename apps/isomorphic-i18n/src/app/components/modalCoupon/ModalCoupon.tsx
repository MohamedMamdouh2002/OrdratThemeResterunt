...
const CouponModal: React.FC<CouponModalProps> = ({ lang, onClose }) => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch(`${API_BASE_URL}/api/Coupon/GetAll/${shopId}`);
  ...
  return (
    ...
    >
      <div
        className={`absolute bottom-0 bg-white h-[400px] w-[320px] sm:w-[400px] shadow-xl transition-transform duration-300 ${
